...
dirin = '/Users/dvimont/Documents/Papers/ENSO/CSLIM_dumb/Code';
...
% Now we load in the scaling factor for the PCs. It contians the square root 
% of the sum of the eigenvalues for the SST and SSH PCs.
scaling = ncread(fullfile(dirin,'state_vector_data.nc'),'scaling');
scale_sst = scaling(1); % scaling for SST
disp(['SST scaling: ', num2str(scale_sst)]);
scale_ssh = scaling(2); % scaling for SSH
disp(['SSH scaling: ', num2str(scale_ssh)]);
...
% Feel free to change this value! This is June, 1997
init_yr = 1997 ; 
init_mon = 6 ; 
x0 = x(12*(init_yr - 1958)+init_mon,:)';
...
for ilead = 1:lead_max
    x_forecast_LIM(:,ilead+1) = expm(L_LIM * ilead) * x_forecast_LIM(:,1);
end
